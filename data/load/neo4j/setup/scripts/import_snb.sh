...
## Gunzip files - find . -type f -name '*.gz' -exec gunzip -f {} \;
...
find /import/${DATASET}/bi-sf${SCALE_FACTOR}-composite-projected-fk/ -type f -name 'part-*.csv' -exec sh -c 'tail -n +2 "$0" > "$0.tmp" && mv "$0.tmp" "$0"' {} \;
